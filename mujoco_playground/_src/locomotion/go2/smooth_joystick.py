...
class Joystick(go2_base.Go2Env):
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    foot_z = foot_pos[..., -1]
    contact = jax.nn.sigmoid((0.01 - foot_z) * 50.0)
    # contact_filt = contact | state.info["last_contact"]
    # contact_filt = jp.maximum(contact, state.info["last_contact"])
    ...
    soft_done = self._get_soft_termination(data)

    rewards = self._get_reward(
        data, action, state.info, state.metrics, soft_done, first_contact, contact
    )
    rewards = {
  ...
  def _get_soft_termination(self, data: mjx.Data) -> jax.Array:
    up_z = self.get_upvector(data)[-1]
    return jax.nn.sigmoid((0.25 - up_z) * 20.0)
